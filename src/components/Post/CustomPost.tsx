...
function CustomPost({
  ...
  return (
    <div key={post.id} className="w-[500px]  flex-wrap  shadow-lg shadow-slate-300/50 flex items-center justify-center mx-1 md:ml-10 flex-col bg-white">
      <div className="w-full py-2 flex items-center justify-between flex-wrap ">
        ...
        <div className="w-auto flex mx-1 items-center justify-center flex-row flex-wrap my-4">
          {post.user === currentUser.id ? <i onClick={() => setShowPopup(true)} className=" cursor-pointer p-1 "> <BiTrash color="#DC2626
" className="hover:text-slate-900" size="1.5em" /></i> : ""}
      ...
      {<div className="w-full py-2  items-start justify-start">
        <PostIcons id={post.id} likes={post.likes} commentIconClick={() => setShowComments(true)} />
        {post.likes && (
        ...
        {showComments && <Comment id={post.id} />}

        {<AddComment handleCommentState={() => setShowComments(true)} id={post.id} />}
